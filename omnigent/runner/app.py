...
def create_runner_app(
    *,
    ...
    async def _ensure_native_terminal_for_turn(conv_id: str, harness_name: str | None) -> None:
        ...
        Every native terminal short-name this can target has a matching
        ``ensure_native_terminal`` branch in ``create_session_terminal`` (kept in
        lockstep with ``harness_aliases.NATIVE_HARNESSES``); a future native
        harness added without that branch would fall through to the generic
        launch — the ``healed`` re-check below turns that into an explicit turn
        failure rather than a silent mis-heal.

        :raises RuntimeError: If the pane cannot be restored to a live state —
            failing the turn explicitly beats forwarding into a dead pane and
        ...
        if terminal_registry is None:
            return
        existing = terminal_registry.get(conv_id, terminal_name, "main")
        # Gate on ``running`` too (consistent with every other registry consumer):
        # a pane whose tmux process died without going through ``close()`` (a
        # crash) is registered-but-dead, and forwarding into it loses the message
        # just like a reaped pane — so heal that case as well.
        if existing is not None and existing.running:
            return  # pane is live — nothing to heal
        _logger.info(
            "native pane missing/dead for conv=%s harness=%s; re-ensuring before turn (#1349)",
            conv_id,
            harness_name,
        )
        try:
            ...
            raise RuntimeError(
                f"native pane self-heal failed for conv={conv_id} ({terminal_name}): {exc}"
            ) from exc
        # Confirm a live pane now exists; otherwise fail the turn explicitly rather
        # than letting the forward inject into a dead tmux target (silent loss).
        status = getattr(resp, "status_code", 200)
        healed = terminal_registry.get(conv_id, terminal_name, "main")
        if status >= 400 or healed is None or not healed.running:
            raise RuntimeError(
                f"native pane self-heal did not restore a live pane for conv={conv_id} "
                f"({terminal_name}, status={status}); failing the turn instead of "
                f"forwarding into a dead pane"
            )
